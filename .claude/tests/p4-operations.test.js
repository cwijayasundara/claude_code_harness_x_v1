...
const { recordProductionFeedback, summarizeProductionFeedback } = require("../lib/production-feedback");
const { analyzeFlakiness, appendSensorHistory, compactHistory, verifySensorHistory } = require("../lib/sensor-operations");
const { applyQuarantine, validateQuarantines } = require("../lib/sensor-quarantine");
...
test("managed Git hooks install without overwriting external hooks", () => {
  const root = project();
  const script = path.resolve(__dirname, "..", "scripts", "harness-git-hooks.js");
